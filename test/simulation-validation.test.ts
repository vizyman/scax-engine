...
describe("시뮬레이션 품질 검증 세트", () => {
  ...
  it("난시 축 180도 주기성(axis, axis+180)에서 simulate 난시 요약이 동일하다", () => {
    const base = createEngine("gullstrand", { s: 0, c: -1.75, ax: 25 });
    const wrapped = createEngine("gullstrand", { s: 0, c: -1.75, ax: 205 });
    const baseMeridians = base.calculateMeridians([{ s: 0, c: -1.75, ax: 25 }]);
    const wrappedMeridians = wrapped.calculateMeridians([{ s: 0, c: -1.75, ax: 205 }]);
    expect(baseMeridians).toEqual(wrappedMeridians);
  });
  ...
  it("넓은 입력 범위에서도 수치가 유한값으로 유지된다", () => {
    ...
    for (let i = 0; i < 30; i += 1) {
      ...
      const combinedMeridians = engine.calculateMeridians([eye, ...lens]);
